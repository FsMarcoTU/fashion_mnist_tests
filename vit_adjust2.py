...
class LearnableFourierLayer(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.apply_fourier = nn.Parameter(torch.tensor(0.5))
        self.linear = nn.Conv2d(channels*2, channels, 1)
    def forward(self, x):
        print(x.shape)
        x_fourier = torch.fft.fft2(x)
        x_complex = torch.cat((x_fourier.real, x_fourier.imag), dim=1)
        x_fourier = self.linear(x_complex)
        print(x_fourier.shape)
        return self.apply_fourier * x_fourier + (1 - self.apply_fourier) * x
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                LearnableFourierLayer(channels=2),
                Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout),
                FeedForward(dim, mlp_dim, dropout = dropout)
            ]))

    def forward(self, x):
        for lf, attn, ff in self.layers:
            x = lf(x)
            x = attn(x) + x
            x = ff(x) + x
