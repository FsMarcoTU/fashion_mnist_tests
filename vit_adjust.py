...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.spatial_patch_embedding = nn.Sequential(
    ...
    def forward(self, img):
        spatial_x = self.spatial_patch_embedding(img[:, :, :self.image_half])
        freq_x = self.freq_patch_embedding(img[:, :, self.image_half:])
        x = torch.cat((spatial_x, freq_x), dim=1)
        b, n, _ = x.shape
